...
const metadataCID = "bafybeif3mdlkguk2vdabfcg7olacoxpngdgztletjibtj73spetzcgqn4a";
const metadataGateway = process.env.REACT_APP_METADATA_GATEWAY;
...
export default class EmojiCoinCert extends Component {
  ...
  componentDidMount() {
    ...
    {
        this.setState({ error : 'emojiCoin id was not provided in URL. Correct format "/preview?id=[IPFS_CID]]"'})
        this.setState({ isLoading: false });
        return;
    }

    const metadataURL = metadataGateway + "/ipfs/" + metadataCID + "/" + this.props.coinId + ".json"; 
    ...
    .catch(errorMsg => {
        this.setState({ error : errorMsg.message, isLoading: false });
    });
  }


  render() {
        const { isLoading, error, isPictureLoaded } = this.state;
        ...
        return (
                <div className="coin-box certBackground">
